...
- **② 高清化（可选）**：一键生成「四格方案对比图」（原图 / Lanczos3 / Real-ESRGAN / Waifu2x 同一局部
  ...
  后续动作预览与动画生成都会**直接使用高清资源（不会重复放大）**；也可「跳过高清化」用原图。
- **③ 编排时间轴**：先「生成动作预览图」把每个动作渲染成一张预览卡（每个动作长什么样一目了然，
  可填「这个动作代表什么」），然后二选一排时间轴：写一句自然语言让 DeepSeek（有 Key）或离线关键词编排；
  或在预览卡上「＋ 加入时间轴」手动勾选；下方 PR 式色块时间轴可改每段时长/循环/倍速、↑↓ 调顺序、✕ 删除。
